...
const CardItem = ({ label, desc, icon: Icon, onClick, color, bg }) => (<div onClick={onClick} className="w-full bg-gray-900/80 border border-gray-800 rounded-2xl p-5 flex items-start gap-4 hover:border-pink-500/50 transition-all active:scale-95 cursor-pointer relative overflow-hidden group"><div className={`absolute inset-0 ${bg} opacity-5 group-hover:opacity-10 transition-opacity`}></div><div className={`p-3 rounded-xl shrink-0 ${bg} ${color}`}><Icon size={24} /></div><div className="flex-1 min-w-0 z-10"><h3 className="text-lg font-bold text-gray-100 leading-tight mb-1">{label}</h3><p className="text-sm text-gray-400 leading-snug">{desc}</p></div></div>);
...
export default function App() {
  ...
  const renderHome = () => (
    <div className="flex flex-col h-full justify-between pt-12 pb-6 animate-fade-in bg-[#0a0a0a] overflow-y-auto">
      ...
      <div className="space-y-6 px-8 pb-8 mt-auto"><Button onClick={() => setScreen('audience')}><Play fill="currentColor" className="w-5 h-5" /> ENTRAR AL JUEGO</Button><div className="text-[10px] text-center text-gray-600 font-mono">v41.0 • RESPONSIVE UNIVERSE<br/><span className="opacity-50">by JTA</span></div></div>
    </div>
  );
